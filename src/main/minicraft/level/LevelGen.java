...
public class LevelGen {
	...
	private static byte[][] createAndValidateSkyMap(int w, int h) {
		...
		do {
			...
			//if (count[Tiles.get("cloud tree").id & 0xff] < 100) continue;
		...
		while (true) {
			...
			for (int y = 0; y < h; y++) {
				for (int x = 0; x < w; x++) {
					...
					if (map[i] == Tiles.get("fir tree").id) pixels[i] = 0x606060;
					if (map[i] == Tiles.get("pine tree").id) pixels[i] = 0x616161;
					if (map[i] == Tiles.get("test").id) pixels[i] = 0x003030;
					if (map[i] == Tiles.get("lawn").id) pixels[i] = 0x009090;
					if (map[i] == Tiles.get("orange tulip").id) pixels[i] = 0x069690;
